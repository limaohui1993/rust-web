...
(pool:&PgPool,teacher_id:i32,course_id:i32)->Result<Course,MyError> {
    ...
    if let Some(course) = row {
        ...
        Err(MyError::NotFount("Course Id not found".into()))
    }
...
(pool:&PgPool,new_course:Course)->Result<Course,MyError> {
    let row=sqlx::query_as!(
        ...
        r#"insert into course (teacher_id, name,description,format,structure,
            duration,price,language,level) values ($1,$2,$3,$4,$5,$6,$7,$8,$9)
        returning id , teacher_id, name,time,description,format,structure,
...
(pool:&PgPool,teacher_id:i32,id:i32,update_course:UpdateCourse)->Result<Course,MyError> {
    ...
    .await
    .map_err(|_err| MyError::NotFount("Course Id not found".into()))?;
    ...
    let course_row=sqlx::query_as!(
        Course,
        "update course set name=$1, description=$2, format=$3, structure=$4, duration=$5,
        level=$6, language=$7, price=$8 where teacher_id=$9 and id=$10
        returning id, teacher_id, name, time,description,format,structure,duration,
